...
class DatabaseManager():
    """This class manage saving to sqlite database"""
    def __init__(self, rooms_file, persons_file, database_name = 'data_files/amity_database.db'):
    ...
    def create_tables(self):
        ...
        self.db_cursor.execute('CREATE TABLE IF NOT EXISTS rooms( id INTEGER\
                                PRIMARY KEY AUTOINCREMENT, room_name TEXT(30),\
                                room_type TEXT (6), no_of_occupants INTEGER (1), \
                                members TEXT )')
        self.db_cursor.execute('CREATE TABLE IF NOT EXISTS persons( id INTEGER \
    ...
    def save_state(self):
        """ave the state of the system to database"""
    ...
    def save_rooms(self):
        ...
        for room_name, room_info in self.rooms_data.items():
            self.db_cursor.execute("SELECT id FROM rooms WHERE room_name = '%s'" % (room_name))
            row_exist=self.db_cursor.fetchall()
            members_list = ', '.join(room_info.members.values()) if room_info.members else 'EMPTY'
            if len(row_exist) == 0:    
                query = "INSERT INTO rooms(room_name, room_type, no_of_occupants, members) \
                                   VALUES ('%s','%s',%d,'%s')" % (room_info.name, room_info.room_type, room_info.no_of_occupants, members_list)
            else:                
                query = "UPDATE rooms SET no_of_occupants = %d, room_type = '%s', members = '%s' WHERE  \
                        room_name = '%s'" % (room_info.no_of_occupants, room_info.room_type, members_list, room_name)
    ...
    def save_persons(self):
        ...
        for person, person_info in self.persons_data.items():
            self.db_cursor.execute("SELECT id FROM persons WHERE person_id = '%s'" % (person))
            row_exist=self.db_cursor.fetchall()
            if len(row_exist) == 0:
    ...
    def load_rooms(self):
        ...
        for row in results:
            ...
            # Create new rooms
            room = Office(room_name) if room_type == 'office' else Living(room_name)
    ...
    def load_persons(self):
        ...
        for row in results:
            ...
            person = Fellow(first_name, last_name) \
                if person_type.upper() == 'FELLOW' else Staff(first_name, last_name)
            person.id = str(row[1])
